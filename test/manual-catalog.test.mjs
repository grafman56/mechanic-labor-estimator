...
test('includes the verified factory-default BMW 325Ci manual in the selectable catalog', async () => {
  const catalog = JSON.parse(await readFile(new URL('../data/lemon-bmw-catalog.json', import.meta.url)));
  assert.ok(catalog.some((entry) => (
    entry.make === 'BMW'
    && entry.year === 2006
    && entry.model === 'BMW 325Ci Convertible (E46)'
    && entry.engine === 'L6-2.5L (M54)'
